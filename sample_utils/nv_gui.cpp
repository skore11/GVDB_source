...
{
	switch ( action ) {
	case 0:		return g_Gui.MouseUp(x,y);		break;
	case 1:		return g_Gui.MouseDown(x,y);	break;
	case 2:		return g_Gui.MouseDrag(x,y);	break;
	}
	return false;
}
...
{	
		...
		for (int n=0; n < GRP_MAX; n++ ) {
			...
			glBufferData ( GL_ARRAY_BUFFER, s.mNum[n] * sizeof(nvVert), s.mGeom[n], GL_DYNAMIC_DRAW_ARB);	
...
void nvDraw::drawSet2D ( nvSet& s )
{
	...
	for (int n=0; n < s.mNum[GRP_IMG] / 4 ; n++ ) {
		glBindTexture ( GL_TEXTURE_2D, s.mImgs[n]->getTex());										
...
void nvDraw::drawSet3D ( nvSet& s )
{
	...
	if ( s.mVBO[GRP_BOX] !=0 && s.mNum[GRP_BOX] != 0 ) {						
		...
		glDrawArraysInstanced ( GL_LINES, 0, mBoxSet.mNum[GRP_LINES], s.mNum[GRP_BOX] );			
...
bool nvDraw::LoadFont ( const char * fontName )
{
	...
	char* srchpath[1];
	srchpath[0] = (char*) malloc ( 1024 );		
	std::string asset_path = std::string(ASSET_PATH);	// asset path provided as define by cmake
	strncpy ( srchpath[0], asset_path.c_str(), 1024 );
	getFileLocation ( fname, fpath, &srchpath[0], 1 );
	free ( srchpath[0] );
...
void nvGui::Draw ( nvImg* chrome )
{
	...
	for (int n=0; n < mGui.size(); n++ ) {
		...
		drawFill ( x1, y1, x2, y2, 0.5, 0.5, 0.5, 0.75 );
	...
	for (int n=0; n < mGui.size(); n++ ) {
		...
		case GUI_CHECK: 
			if ( x > x1 && x < x2 && y > y1 && y < y2 ) {
				...
				int val;
				switch ( mGui[ n ].dtype ) {
				case GUI_INT:	val = ( (*(int*) mGui[n].data) == 0 ) ? 1 : 0;			*(int*) mGui[n].data = val;		break;
				case GUI_FLOAT:	val = ( (*(float*) mGui[n].data) == 0.0 ) ? 1.0 : 0.0;	*(float*) mGui[n].data = val;	break;
				case GUI_BOOL:	val = ( (*(bool*) mGui[n].data) == false ) ? 1 : 0;		*(bool*) mGui[n].data = val;	break;
				};
				if ( g_GuiCallback ) g_GuiCallback( n, val );
				return true;
			}
			break;
		case GUI_COMBO:
...
bool nvGui::MouseDrag ( float x, float y )
{
	...
	if ( mActiveGui != -1 ) {
		...
		if ( x <= x3 ) {
			...
			case GUI_INT:	val = (int) mGui[ mActiveGui ].vmin; break;
			case GUI_FLOAT:	val = (float) mGui[ mActiveGui ].vmin; break;						
			};
		...
		if ( x >= x2 ) {
			mGui[ mActiveGui ].changed = true;
			switch ( mGui[ mActiveGui ].dtype ) {
			case GUI_INT:	val = (int) mGui[ mActiveGui ].vmax;	break;
			case GUI_FLOAT:	val = (float) mGui[ mActiveGui ].vmax;	break;						
			};
			if ( g_GuiCallback ) g_GuiCallback ( mActiveGui, val );
			return true;
		}
		if ( x > x3 && x < x2 ) {
			mGui[ mActiveGui ].changed = true;
			switch ( mGui[ mActiveGui ].dtype ) {
			case GUI_INT:	val = (int) (mGui[ mActiveGui ].vmin + (x-x3)*mGui[ mActiveGui ].vmax / (x2-x3));	 break;
