...
FUNCTION( _COMPILEPTX )
  set(options "")
  set(oneValueArgs TARGET_PATH GENERATED)
  ...
  if ( WIN32 ) 
		...
		FOREACH( input ${_COMPILEPTX_SOURCES} )
			...
			if ( ${input_ext} STREQUAL ".cu" )			
				...
				LIST( APPEND PTX_FILES ${output_with_path} )									# Append to output list
    
				message( STATUS "NVCC Compile: ${CUDA_NVCC_EXECUTABLE} ${MACHINE} --ptx ${_COMPILEPTX_OPTIONS} ${input} -o ${output_with_path} WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}")
    
				add_custom_command(
					OUTPUT  ${output_with_path}
					MAIN_DEPENDENCY ${input}
					COMMAND ${CUDA_NVCC_EXECUTABLE} ${MACHINE} --ptx ${_OPTS} ${input} -o ${output_with_path} WORKING_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
				)			
			endif()
		ENDFOREACH( )
  else ()
...
function( _INSTALL )   
  ...
  if ( WIN32 )      
      ...
      if ( _INSTALL_SOURCE )	   
	     foreach ( _file ${_INSTALL_FILES} )
	         list ( APPEND _instlist "${_INSTALL_SOURCE}${_file}" )
		 endforeach()
      else()
	     set ( _instlist ${_INSTALL_FILES} )
      endif() 
      install ( FILES ${_instlist} DESTINATION ${_INSTALL_DESTINATION} )        
  endif( )
endfunction()
