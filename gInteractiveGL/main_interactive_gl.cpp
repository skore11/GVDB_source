...
bool Sample::init() 
{
	...
	// Set volume params
	gvdb.getScene()->SetSteps ( 1.0, 16, 0.8 );				// Set raycasting steps
	...
	gvdb.getScene()->LinearTransferFunc ( 0.00f, 0.25f, Vector4DF(0,0,0,0.0), Vector4DF(1,1,0,0.1) );
	gvdb.getScene()->LinearTransferFunc ( 0.25f, 0.50f, Vector4DF(1,1,0,0.4), Vector4DF(1,0,0,0.3) );
	gvdb.getScene()->LinearTransferFunc ( 0.50f, 0.75f, Vector4DF(1,0,0,0.3), Vector4DF(.2,.2,0.2,0.1) );
	gvdb.getScene()->LinearTransferFunc ( 0.75f, 1.00f, Vector4DF(.2,.2,0.2,0.1), Vector4DF(0,0,0,0.0) );
...
{
	Sample sample_obj;
	return sample_obj.run ( "gInteractveGL", argc, argv, 1024, 768, 4, 5 );
}
