/*
  ...
  Attribute-ZLib license (*See part 4)
...
void FluidSystem::Initialize ()
{
	...
	LoadKernel ( FUNC_PREFIXSUM,		"prefixSum" );
	LoadKernel ( FUNC_PREFIXFIXUP,		"prefixFixup" );
...
void FluidSystem::Start ( int num )
{
	...
	SetupDefaultParams ();	
	SetupExampleParams ();	
	m_Param[PNUM] = num;	// maximum number of points
	mMaxPoints = num;
...
void FluidSystem::SetupAddVolume ( Vector3DF min, Vector3DF max, float spacing, float offs, int total )
{
	...
	for (float y = min.y; y <= max.y; y += spacing ) {	
		for (int xz=0; xz < cnt; xz++ ) {
			...
			p = AddParticle ();
			if ( p != -1 ) {
				(m_Fluid.bufV3(FPOS)+p)->Set ( x,y,z);
				
				Vector3DF clr ( (x-min.x)/dx, 0, (z-min.z)/dz );				
				clr *= 0.8; clr += 0.2;				
				clr.Clamp (0, 1.0);
...
void FluidSystem::SetupExampleParams ()
{
	...
	case 1:		// Wave pool						
		...
		m_Vec [ PVOLMAX ].Set (  100, 150, 100 );
		m_Vec [ PINITMIN ].Set ( -90, 30, -90 );
		m_Vec [ PINITMAX ].Set (  90, 140, 90 );
		m_Param [ PFORCE_MIN ] = 40.0f;	
		m_Param [PFORCE_FREQ ] = 16.0f;
		m_Param [ PGROUND_SLOPE ] = 0.15f;
		break;
	case 2:		// Large coast						
...
void FluidSystem::FluidParamCUDA ( float ss, float sr, float pr, float mass, float rest, float3 bmin, float3 bmax, float estiff, float istiff, float visc, float damp, float fmin, float fmax, float ffreq, float gslope, float gx, float gy, float gz, float al, float vl, int emit )
{
	...
	m_FParams.poly6kern = 315.0f / (64.0f * 3.141592 * pow( sr, 9.0f) );
	m_FParams.spikykern = -45.0f / (3.141592 * pow( sr, 6.0f) );
	m_FParams.lapkern = 45.0f / (3.141592 * pow( sr, 6.0f) );	
	m_FParams.gausskern = 1.0 / pow(3.141592 * 2.0*sr*sr, 3.0/2.0);
...
void FluidSystem::PrefixSumCellsCUDA ( uint* goff, int zero_offsets )
{
		...
		if ( numElem1 > SCAN_BLOCKSIZE*ulong(SCAN_BLOCKSIZE)*SCAN_BLOCKSIZE) {
		...
		cuCheck ( cuLaunchKernel ( m_Func[FUNC_PREFIXSUM], numElem2, 1, 1, threads, 1, 1, 0, NULL, argsA, NULL ), "cuPrefixSumA" );		

		void* argsB[5] = { &array2, &scan2, &array3, &numElem2, &zon }; // sum array2. output -> scan2, array3
		cuCheck ( cuLaunchKernel ( m_Func[FUNC_PREFIXSUM], numElem3, 1, 1, threads, 1, 1, 0, NULL, argsB, NULL ), "cuPrefixSumB" );		

		if ( numElem3 > 1 ) {
			CUdeviceptr nptr = {0};
			void* argsC[5] = { &array3, &scan3, &nptr, &numElem3, &zon };	// sum array3. output -> scan3
			cuCheck ( cuLaunchKernel ( m_Func[FUNC_PREFIXSUM], 1, 1, 1, threads, 1, 1, 0, NULL, argsC, NULL ), "cuPrefixSumB" );					

			void* argsD[3] = { &scan2, &scan3, &numElem2 };	// merge scan3 into scan2. output -> scan2
			cuCheck ( cuLaunchKernel ( m_Func[FUNC_PREFIXFIXUP], numElem3, 1, 1, threads, 1, 1, 0, NULL, argsD, NULL ), "cuPrefixFixupC" );			
		}

		void* argsE[3] = { &scan1, &scan2, &numElem1 };		// merge scan2 into scan1. output -> scan1
		cuCheck ( cuLaunchKernel ( m_Func[FUNC_PREFIXFIXUP], numElem2, 1, 1, threads, 1, 1, 0, NULL, argsE, NULL ), "cuPrefixFixupC" );		
