...
inline __device__ float3 getAtlasToWorld ( uint3 vox )
{
	int3 i = make_int3(vox.x/gvdb.brick_res, vox.y/gvdb.brick_res, vox.z/gvdb.brick_res );
	int3 p = make_int3(vox.x % gvdb.brick_res, vox.y % gvdb.brick_res, vox.z % gvdb.brick_res ) - make_int3(gvdb.atlas_apron);			
	int id = (i.z*gvdb.atlas_cnt.y + i.y )*gvdb.atlas_cnt.x + i.x;		// brick id			
	return ( make_float3((gvdb.atlas_map + id)->mPos) + make_float3(p) + make_float3(0.5,0.5,0.5) ) * gvdb.voxelsize;
}
