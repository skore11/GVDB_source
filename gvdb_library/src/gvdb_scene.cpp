...
void Scene::UpdateTransferFromImg ()
{
	/*Vector4DF clr, clr1, clr2;

	int sz = 16384*sizeof(Vector4DF);
	if ( mTransferFunc != 0x0 ) free ( mTransferFunc );
	mTransferFunc = (Vector4DF*) malloc ( sz );

	unsigned char* pix = mTransferImg.getData();
	int w = mTransferImg.getWidth ();
	int lasty = mTransferImg.getSize() - w*4;
	float x;
	int xi;
	
	// remap transfer image to fixed array of float values
	for (int n=0; n < 16384; n++ ) {
		x = (float) n * (float) w / 16384.0;
		xi = int(x);

		clr1.x = float(*(pix + xi*4+0)) / 255.0;
		clr1.y = float(*(pix + xi*4+1)) / 255.0;
		clr1.z = float(*(pix + xi*4+2)) / 255.0;
		clr1.w = float(*(pix + xi*4+0 + lasty)) / 255.0;
		
		clr2.x = float(*(pix + (xi+1)*4+0)) / 255.0;
		clr2.y = float(*(pix + (xi+1)*4+1)) / 255.0;
		clr2.z = float(*(pix + (xi+1)*4+2)) / 255.0;
		clr2.w = float(*(pix + (xi+1)*4+0 + lasty)) / 255.0;

		clr = lerp4 ( clr1, clr2, x-float(xi) );

		mTransferFunc[n].Set ( clr.x, clr.y, clr.z, clr.w );
	}
	mTransferFunc[0].Set ( 0, 0, 0, 0 );
	mTransferFunc[16383].Set ( 0, 0, 0, 0 );	*/
}
...
void Scene::AddTransfer ( std::string filestr, Vector3DF tvec )
{
	char fpath[1024];	
	if ( !FindFile ( filestr, fpath ) ) {
		gprintf ( "ERROR: Unable to open '%s'\n", filestr.c_str() ); 
		gerror ();
	}
	//mTransferImg.LoadPng ( fpath );
	//mTransferImg.FlipY ();
	mTransferVec = tvec;
}
...
/*gParse->GetToken( val );	gScene->mVLeaf.x = atof(val);	
	gParse->GetToken( val );	gScene->mVLeaf.y = atof(val);	
	gParse->GetToken( val );	gScene->mVLeaf.z = atof(val);*/
...
void Scene::VolumeTransfer ()
{
	char val[512];
	char fname[512];	
	Vector3DF vec;
	gParse->GetToken( fname );		// transfer function image
	gParse->GetToken( val );	vec.x = atof(val);	
	gParse->GetToken( val );	vec.y = atof(val);	
	gParse->GetToken( val );	vec.z = atof(val);	
	gScene->AddTransfer ( fname, vec );
}
...
void Scene::LoadFile ( std::string filestr  )
{
	...
	gParse->SetCallback( "vtransfer",       &Scene::VolumeTransfer );
