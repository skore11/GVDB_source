...
void Sample::RebuildOptixGraph ()
{
	...
	matp->diff_color = Vector3DF(.3, .34, .3);
	matp->spec_color = Vector3DF(.3, .3, .3 );
	matp->spec_power = 40.0;
	matp->env_color  = Vector3DF(.1, .1, .1 );
	matp->refl_width = 5.0;
	matp->refl_color = Vector3DF(3.0, 3.0, 3.0 );
	matp->refr_width = 0.5;
	matp->refr_color = Vector3DF(.35, .4, .4 );
	matp->refr_ior = 1.2;
	matp->refr_amount = 1.0;
	matp->refr_offset = 15.0;
...
bool Sample::init() 
{
	...
	// Set volume params
	gvdb.getScene()->SetSteps ( 2.0, 16, 0.1 );				// Set raycasting steps
	...
	gvdb.getScene()->SetBackgroundClr ( 0.1, 0.2, 0.4, 1 );
...
{
	Sample sample_obj;
	return sample_obj.run ( "gInteractveOptix", argc, argv, 1024, 768, 4, 5 );
}
